...
const aiMax = computed(() => Math.max(...aiStats.map((a) => a.count)));
...
function rankBarColor(i: number) {
...
<template>
  <div class="ds-body">
    ...
    <div class="ds-col ds-col-left">
      ...
      <section class="ds-card">
        <div class="ds-card-title"><span class="ds-title-bar" />工单闭环</div>
        <div class="ds-wo-row">
          <div class="ds-wo-ring">
            <svg viewBox="0 0 36 36" class="ds-wo-svg">
              <path d="M18 2.0845a 15.9155 15.9155 0 0 1 0 31.831 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="rgba(255,255,255,0.06)" stroke-width="3" />
              <path d="M18 2.0845a 15.9155 15.9155 0 0 1 0 31.831 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="#10b981" stroke-width="3" :stroke-dasharray="`${woKpis.closedRate}, 100`" stroke-linecap="round" />
            </svg>
            <span class="ds-wo-ring-val">{{ woKpis.closedRate }}%</span>
          </div>
          <div class="ds-wo-detail">
            <div class="ds-wo-item"><span class="ds-wo-label">总工单</span><span class="ds-wo-val">{{ woKpis.total }}</span></div>
            <div class="ds-wo-item"><span class="ds-wo-label">已闭环</span><span class="ds-wo-val" style="color:#10b981">{{ woKpis.closed }}</span></div>
            <div class="ds-wo-item"><span class="ds-wo-label">待处理</span><span class="ds-wo-val" style="color:#f59e0b">{{ woKpis.pending }}</span></div>
            <div class="ds-wo-item"><span class="ds-wo-label">平均处置</span><span class="ds-wo-val">{{ woKpis.avgHours }}h</span></div>
          </div>
        </div>
      </section>
      ...
      <section class="ds-card">
        ...
        <div class="ds-rank-list">
          <div v-for="(r, i) in deptRank" :key="r.label" class="ds-rank-row">
            ...
            <div class="ds-prog-track">
              <div class="ds-prog-fill" :style="{ width: r.pct + '%', background: rankBarColor(i) }" />
            </div>
